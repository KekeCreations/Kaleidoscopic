...
public class KBlockTagProvider extends FabricTagsProvider.BlockTagsProvider{


    public KBlockTagProvider(FabricPackOutput output, CompletableFuture<HolderLookup.Provider> registryLookupFuture) {
    ...
    private void rock() {
        for (DyeColor colour : DyeColor.values()) {
            if (colour.getId() > DyeColor.BLACK.getId()) {
                this.valueLookupBuilder(KTags.BlockTags.ROCK)
                        .addOptional(getOpt(KBlocks.DYED_ROCKS.get(colour).get()));
            }
            else {
                this.valueLookupBuilder(KTags.BlockTags.ROCK)
    ...
    private void rockChiseled() {
        for (DyeColor colour : DyeColor.values()) {
            if (colour.getId() > DyeColor.BLACK.getId()) {
                this.valueLookupBuilder(KTags.BlockTags.CHISELED_ROCK)
                        .addOptional(getOpt(KBlocks.CHISELED_DYED_ROCKS.get(colour).get()));
            }
            else {
                this.valueLookupBuilder(KTags.BlockTags.CHISELED_ROCK)
    ...
    private void rockStairs() {
        for (DyeColor colour : DyeColor.values()) {
            if (colour.getId() > DyeColor.BLACK.getId()) {
                this.valueLookupBuilder(KTags.BlockTags.ROCK_STAIRS)
                        .addOptional(getOpt(KBlocks.DYED_ROCK_STAIRS.get(colour).get()));
            }
            else {
                this.valueLookupBuilder(KTags.BlockTags.ROCK_STAIRS)
    ...
    private void rockSlab() {
        for (DyeColor colour : DyeColor.values()) {
            if (colour.getId() > DyeColor.BLACK.getId()) {
                this.valueLookupBuilder(KTags.BlockTags.ROCK_SLAB)
                        .addOptional(getOpt(KBlocks.DYED_ROCK_SLABS.get(colour).get()));
            }
            else {
                this.valueLookupBuilder(KTags.BlockTags.ROCK_SLAB)
    ...
    private void rockWall() {
        for (DyeColor colour : DyeColor.values()) {
            if (colour.getId() > DyeColor.BLACK.getId()) {
                this.valueLookupBuilder(KTags.BlockTags.ROCK_WALL)
                        .addOptional(getOpt(KBlocks.DYED_ROCK_WALLS.get(colour).get()));
            }
            else {
                this.valueLookupBuilder(KTags.BlockTags.ROCK_WALL)
    ...
    private void rockBricks() {
        for (DyeColor colour : DyeColor.values()) {
            if (colour.getId() > DyeColor.BLACK.getId()) {
                this.valueLookupBuilder(KTags.BlockTags.ROCK_BRICKS)
                        .addOptional(getOpt(KBlocks.DYED_ROCK_BRICKS.get(colour).get()));
            }
            else {
                this.valueLookupBuilder(KTags.BlockTags.ROCK_BRICKS)
    ...
    private void rockBrickStairs() {
        for (DyeColor colour : DyeColor.values()) {
            if (colour.getId() > DyeColor.BLACK.getId()) {
                this.valueLookupBuilder(KTags.BlockTags.ROCK_BRICK_STAIRS)
                        .addOptional(getOpt(KBlocks.DYED_ROCK_BRICK_STAIRS.get(colour).get()));
            }
            else {
                this.valueLookupBuilder(KTags.BlockTags.ROCK_BRICK_STAIRS)
    ...
    private void rockBrickSlab() {
        for (DyeColor colour : DyeColor.values()) {
            if (colour.getId() > DyeColor.BLACK.getId()) {
                this.valueLookupBuilder(KTags.BlockTags.ROCK_BRICK_SLAB)
                        .addOptional(getOpt(KBlocks.DYED_ROCK_BRICK_SLABS.get(colour).get()));
            }
            else {
                this.valueLookupBuilder(KTags.BlockTags.ROCK_BRICK_SLAB)
    ...
    private void rockBrickWall() {
        for (DyeColor colour : DyeColor.values()) {
            if (colour.getId() > DyeColor.BLACK.getId()) {
                this.valueLookupBuilder(KTags.BlockTags.ROCK_BRICK_WALL)
                        .addOptional(getOpt(KBlocks.DYED_ROCK_BRICK_WALLS.get(colour).get()));
            }
            else {
                this.valueLookupBuilder(KTags.BlockTags.ROCK_BRICK_WALL)
    ...
    private void ladders() {
        for (DyeColor colour : DyeColor.values()) {
            if (colour.getId() > DyeColor.BLACK.getId()) {
                this.valueLookupBuilder(KTags.BlockTags.DYED_LADDERS)
                        .addOptional(getOpt(KBlocks.DYED_LADDERS.get(colour).get()));
            }
            else {
                this.valueLookupBuilder(KTags.BlockTags.DYED_LADDERS)
    ...
    private void doors() {
        for (DyeColor colour : DyeColor.values()) {
            if (colour.getId() > DyeColor.BLACK.getId()) {
                this.valueLookupBuilder(KTags.BlockTags.DYED_DOORS)
                        .addOptional(getOpt(KBlocks.DYED_DOORS.get(colour).get()));
            }
            else {
                this.valueLookupBuilder(KTags.BlockTags.DYED_DOORS)
    ...
    private void trapdoors() {
        for (DyeColor colour : DyeColor.values()) {
            if (colour.getId() > DyeColor.BLACK.getId()) {
                this.valueLookupBuilder(KTags.BlockTags.DYED_TRAPDOORS)
                        .addOptional(getOpt(KBlocks.DYED_TRAPDOORS.get(colour).get()));
            }
            else {
                this.valueLookupBuilder(KTags.BlockTags.DYED_TRAPDOORS)
    ...
    private Block getOpt(Block block) {
        return BuiltInRegistries.BLOCK.getValue(block.properties().blockId());
    }
}
